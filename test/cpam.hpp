		...
		ptr_adapter<UPtr>, size_t, ANN::util::empty>
{
  ...
  */
  ...
  bool operator==(const ptr_adapter &rhs) const {
    return static_cast<const base_ptr &>(*this) == static_cast<const base_ptr &>(rhs) &&
           static_cast<const base_iter &>(*this) == static_cast<const base_iter &>(rhs);
  }
  bool operator!=(const ptr_adapter &rhs) const {
    return !operator==(rhs);
  }
  bool operator==(const nullptr_t &null) const {
    return base_iter::get_pos() == 0 && static_cast<const base_ptr &>(*this) == null;
  }
  bool operator!=(const nullptr_t &null) const {
...
class shared_vector {
  ...
  void push_back_impl(V &&value) {
    ...
    T *p = s_new.operator->();
    for (auto it = begin(); it != end(); ++it, ++p) new (p) T(*it);
    new (p) T(std::forward<V>(value));
    len++;
    if (storage != nullptr) alloc.deallocate(storage, cap);
 ...
 public:
  ...
  shared_vector(Iter start, Iter last) : len(std::distance(start, last)), cap(len * over_resv) {
    if (len == 0) return;

    storage = alloc.allocate(cap);
    T *p = storage.operator->();
    for (; start != last; ++start, ++p) new (p) T(*start);
  }
...
class graph_cpam : ANN::graph::base {
 ...
 private:
  ...
  class shared_wrapper : public std::shared_ptr<T> {
   ...
   public:
    ...
    shared_wrapper(Args &&...args)
        : base(std::allocate_shared<T>(typename cm::alloc<T>(), std::forward<Args>(args)...)) {}
  };
  ...
  class const_edge_agent {
    ...
    const_edge_agent(const edgelist &other) : el(other) {}
    const_edge_agent(edgelist &&other) : el(std::move(other)) {}
  ...
  struct edge_agent : const_edge_agent {
    ...
    edge_agent &operator=(R &&r) {
      ...
      curr.clear();
      for (const Edge &u : es) curr.insert(u);

      size_t len_r = 0;
      for (const Edge &c : r)
        if (curr.find(c) == curr.end()) len_r++;

      if (len_r + es.size() <= es.capacity()) {
        for (const Edge &c : r)
          if (curr.find(c) == curr.end()) es.push_back(c);
      } else
        es = ANN::util::to<edgelist>(std::forward<R>(r));
  ...
  class vinfo_trivial_ext : public vinfo_edges {
    constexpr static const bool has_default = std::is_default_constructible_v<ext_t>;

   public:
    using raw_t = std::conditional_t<has_default, ext_t, std::optional<ext_t>>;
 ...
 public:
  ...
  struct node_cptr : ptr_base<true> {
    ...
    node_cptr(const node_ptr &other) : ptr_base<true>(other.content) {}
    node_cptr(node_ptr &&other) : ptr_base<true>(std::move(other.content)) {}
  };
 ...
 public:
  ...
  graph_cpam(graph_cpam &&other) noexcept : nodes(std::move(other.nodes)) {}
  ...
  void set_edges(Iter begin, Iter end) {
    ...
    auto vs_delayed = ANN::util::delayed_seq(n, [&](size_t i) {
      ...
      if constexpr (std::is_base_of_v<const_edge_agent, ea_t>)
        return entry_t(u, vinfo(std::forward<decltype(ea)>(ea).get_ref()));
      else  // a range type
        return entry_t(u, vinfo(ANN::util::to<edgelist>(std::forward<decltype(ea)>(ea))));
    });
  ...
  void set_edges(Seq &&ps) {
    if constexpr (std::is_rvalue_reference_v<Seq &&>) {
      ...
      set_edges(std::make_move_iterator(ps.begin()), std::make_move_iterator(ps.end()));
    } else
      set_edges(ps.begin(), ps.end());
  }
  ...
  void iter_each(F &&f) const {
    auto g = [&](const auto &entry) { f(node_cptr(map_entry::get_val(entry))); };
    nodemap::foreach_seq(nodes, g);
  }
  template<class F>
  void iter_each(F &&f) {
    auto g = [&](const auto &entry) { f(node_ptr(map_entry::get_val(entry))); };
    nodemap::foreach_seq(nodes, g);
  }
  template<class F>
  void for_each(F &&f) const {
    auto g = [&](const auto &entry, size_t /*pos*/) { f(node_cptr(map_entry::get_val(entry))); };
    nodemap::foreach_index(nodes, g);
  }
  template<class F>
  void for_each(F &&f) {
    auto g = [&](const auto &entry, size_t /*pos*/) { f(node_ptr(map_entry::get_val(entry))); };
    nodemap::foreach_index(nodes, g);
  }
  template<class F>
  void for_each_raw(F &&f) const {
    auto g = [&](auto &&...args) { f(std::forward<decltype(args)>(args)...); };
