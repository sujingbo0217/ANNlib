...
void run_post_hnsw(uint32_t dim, float m_l, uint32_t m, uint32_t efc, float alpha,
                     float batch_base, uint32_t k, uint32_t ef, size_t size_init, size_t size_step,
                     size_t size_max, auto ps, auto q, auto F_b, auto F_q) {
  ...
  auto res = post_processing<U>(layers, q, k, ef, F_b, F_q);
