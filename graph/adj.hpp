...
namespace ANN::graph {
  ...
  class adj_base : base {
   ...
   public:
    ...
    struct node_cptr : ptr_base<true> {
      ...
      node_cptr(const node_ptr &other) : ptr_base<true>(other.raw) {}
    };
   ...
   protected:
    ...
    Seq pop_edges_impl(node_ptr p, util::dummy<Seq>) {
      ...
      Seq edges(std::make_move_iterator(nbhs.begin()), std::make_move_iterator(nbhs.end()));
      nbhs.clear();
      return edges;
    }
  ...
  class adj_seq : public adj_base<Nid, Ext, Edge, TMapV, TSeqE> {
   ...
   private:
    ...
    node_ptr add_node_impl(nid_t nid, T &&ext) {
      if (nodes.size() < nid + 1) nodes.resize(nid + 1);
   ...
   public:
    ...
    void add_nodes(Iter begin, Iter end) {
      ...
      const auto n = std::distance(begin, end);
      auto nids = util::delayed_seq(n, [&](size_t i) { return (nid_t)std::get<0>(*(begin + i)); });
      nid_t nid_max = *cm::max_element(nids.begin(), nids.end());
      if (nodes.size() < nid_max + 1) nodes.resize(nid_max + 1);
    ...
    template<class F>
    void iter_each(F &&f) {
      for (auto &u : nodes) f(gen_node_ptr(&u));
    }
    template<class F>
    ...
    void for_each_nid(F &&f) {
      cm::parallel_for(0, nodes.size(), [&, it = nodes.begin()](size_t i) { f(nid_t(i)); });
    }
  };
  ...
  class adj_map : public adj_base<Nid, Ext, Edge, TMapV, TSeqE> {
   ...
   public:
    ...
    template<class F>
    void iter_each(F &&f) const {
      util::iter_each(nodes, [&](const auto &p) { f(gen_node_cptr(&p.second)); });
    }
    template<class F>
    void iter_each(F &&f) {
      util::iter_each(nodes, [&](auto &p) { f(gen_node_ptr(&p.second)); });
    }
    template<class F>
    void iter_each_nid(F &&f) {
      for (auto [k, v] : nodes) {
    ...
    template<class F>
    void for_each(F &&f) const {
      util::for_each(nodes, [&](const auto &p) { f(gen_node_cptr(&p.second)); });
    }
    template<class F>
    void for_each(F &&f) {
      util::for_each(nodes, [&](auto &p) { f(gen_node_ptr(&p.second)); });
    }
    template<class F>
    void for_each_nid(F &&f) {
