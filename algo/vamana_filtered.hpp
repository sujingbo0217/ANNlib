...
namespace ANN {
  ...
  template<class Desc>
  template<typename Iter, class Label>
  void filtered_vamana<Desc>::insert(Iter begin, Iter end, const Label &F,
    ...
    auto f_full_intermediate = util::delayed_seq(F.size(), [&](size_t i) {
      std::vector<std::pair<label_t, nid_t>> transformed_vec;
      for (const auto &l : F[perm[i]]) {
        transformed_vec.emplace_back(l, i);
      }
      return transformed_vec;
    });
    auto f_full = cm::flatten(f_full_intermediate);
    ...
    */
    ...
    while (batch_end < n) {
      ...
      auto new_labels = util::delayed_seq(batch_end - batch_begin,
                                          [&](size_t i) { return F[perm[i + batch_begin]]; });
      // insert_batch_impl(rand_seq.begin() + batch_begin, rand_seq.begin() + batch_end,
      // new_labels/*, filtered*/);
      insert_batch_impl(rand_seq.begin() + batch_begin, rand_seq.begin() + batch_end,
                        new_labels /*, filtered*/);
  ...
  template<class Desc>
  void filtered_vamana<Desc>::insert(const std::pair<pid_t, nid_t> &ids, const coord_t &coord,
                                     const std::vector<label_t> &F) {
  ...
  void filtered_vamana<Desc>::insert_batch_impl(Iter begin, Iter end,
    ...
    cm::parallel_for(0, batch_size, [&](size_t i) {
      ...
      pctrl.alpha = alpha;
      seq_conn conn_u = algo::prune_heuristic(std::move(res), get_deg_bound(), gen_f_nbhs(),
                                              gen_f_dist(u), pctrl);
