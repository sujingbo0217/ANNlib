...
namespace ANN {
  ...
  class vamana {
    ...
    */
    seq<edge> edge_cast(const seq<conn> &cs) {
      ...
      auto es = util::delayed_seq(cs.size(), [&](size_t i) { return edge{cs[i], deltick}; });
  ...
  void vamana<Desc>::insert_batch_impl(Iter begin, Iter end) {
    ...
    /*	auto coord_drift = cm::reduce(
                    ...
                    std::views::transform([&](auto &&p){return
       md_t(util::inner_t{},p.get_coord(),dim);})
            );*/
    auto it_refs = util::delayed_seq(size_batch, [&](size_t i) { return begin + i; });
    auto refs_new = util::filter(it_refs, [&](Iter it) { return !!id_map.find_nid(it->get_id()); });
    auto coords_new = util::delayed_seq(refs_new.size(), [&](size_t i) {
      return md_t(util::inner_t{}, refs_new[i]->get_coord(), dim);
    });
    ...
    cm::parallel_for(0, size_batch, [&](size_t i) {
      ...
      pctrl.alpha = alpha;
      seq<conn> conn_u = algo::prune_heuristic(std::move(res), get_deg_bound(), gen_f_nbhs(),
                                               gen_f_dist(u), pctrl);
      // record the edge for the backward insertion later
      auto &edge_cur = edge_added[i];
      edge_cur.clear();
      edge_cur.reserve(conn_u.size());
      for (const auto &[d, v] : conn_u) edge_cur.emplace_back(v, edge{{d, u}, deltick});
    ...
    cm::parallel_for(0, edge_added_grouped.size(), [&](size_t j) {
      ...
      auto edge_v =
          util::to<seq<edge>>(std::move(edge_agent_v) | std::views::filter([&](const edge &e) {
    ...
    ep = algo::beamSearch(
             gen_f_nbhs(),
             [&](nid_t v) { return Desc::distance(t.data(), g.get_node(v)->get_coord(), dim); },
             seq<nid_t>{ep}, 1)[0]
             .u;
  }
  ...
  void vamana<Desc>::erase(Iter begin, Iter end) {
    static_assert(std::is_convertible_v<typename std::iterator_traits<Iter>::value_type, pid_t>);
    /*
    ...
    ep = algo::beamSearch(
             gen_f_nbhs(),
             [&](nid_t v) { return Desc::distance(t.data(), g.get_node(v)->get_coord(), dim); },
             seq<nid_t>{id_map.front_nid()}, 1)[0]
             .u;
  }
