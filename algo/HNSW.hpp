...
namespace ANN {
  ...
  class HNSW {
    ...
    */
    HNSW(uint32_t dim, float m_l = 0.4, uint32_t m = 100, uint32_t efc = 50, float alpha = 1.0);
    /*
   ...
   public:
    ...
    size_t num_edges(uint32_t l, nid_t u) const {
      auto num_edges_impl = [u](const auto &g) { return g.get_edges(u).size(); };
    ...
    size_t max_deg(uint32_t l) const {
      return calc_degs(l, [](size_t x, size_t y) { return std::max(x, y); });
    }
  ...
  template<class Desc>
  HNSW<Desc>::HNSW(uint32_t dim_, float m_l_, uint32_t m_, uint32_t efc, float alpha)
  ...
  void HNSW<Desc>::insert(Iter begin, Iter end, float batch_base) {
    ...
    if (layer_b.empty()) {
      ...
      if (level_ep > 0) {
        ...
        for (uint32_t l = level_ep; l > 0; --l) layer_u[l].add_node(ep_init, node_lite{});
      }
  ...
  void HNSW<Desc>::insert_batch_impl(Iter begin, Iter end) {
    ...
    for (uint32_t l = level[pos_split.back()]; l > 0; --l) add_to_upper(l, size_batch);
    ...
    for (int32_t l = std::min(level_ep, level_max); l >= 0; --l)  // TODO: fix the type
    {
      ...
      size_t pos_end;
      while (pos_end = pos_split[j + 1], pos_end < size_batch && level[pos_end] >= l) j++;
      ...
      auto add_rev_nbhs = [&](auto &g) {
        ...
        cm::parallel_for(0, edge_added_grouped.size(), [&](size_t j) {
          ...
          seq<conn> conn_v = algo::prune_simple(conn_cast(std::move(edge_v)), get_deg_bound(l));
