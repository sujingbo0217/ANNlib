  ...
  class custom<custom_tag_undef> {
   public:
    ...
    template<class R,  // TODO: shorten
             class T = std::remove_reference_t<
                 std::ranges::range_value_t<typename std::remove_reference_t<R>>>,
             class BinaryOp = std::plus<>>
    ...
    static auto flatten(const R &r) {
      ...
      S res;
      for (const auto &e : r) res.insert(res.end(), e.begin(), e.end());
      return res;
    }
    ...
    static auto group_by_key(R r) {
      ...
      sort(r.begin(), r.end(),
           [](const E &x, const E &y) { return std::get<0>(x) < std::get<0>(y); });
      ...
      for (size_t i = 1; i < n; ++i) {
        if (keys[split.back()] != keys[i]) split.push_back(i);
      }
      split.push_back(n);
      ...
      for (size_t i = 1; i < split.size(); ++i)
        res.push_back(
            {keys[split[i - 1]], seq<V>(values.begin() + split[i - 1], values.begin() + split[i])});
      return res;
    }
  };
